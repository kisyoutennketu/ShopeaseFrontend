    ...
    <div className='px-8 w-full lg:w-[70%]'>
      ...
      <div className='pt-4'>
        <form onSubmit={onSubmit}>
          ...
          <button className='border w-full rounded-lg h-[48px] mb-4 bg-black text-white mt-4 hover:opacity-80 '>Sing In</button>
        </form>
      </div>
      {error && <p className='text-lg text-red-700'>{error}</p>}
      <Link to={"/v1/register"} className='underline text-gray-500 hover:text-black '>Don't have an account? Sing up</Link>
    </div>
  )
}
